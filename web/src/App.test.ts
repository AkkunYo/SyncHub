...
describe('SyncHub console', () => {
  ...
  it('retries a live channel failure, then edits and deletes channels', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '危险删除原生渠道 Native channel' }))
    const deleteDialog = screen.getByRole('dialog', { name: '危险操作：删除目标原生渠道' })
    const confirmDelete = within(deleteDialog).getByRole('button', { name: '确认直接删除' })
    expect(confirmDelete).toBeDisabled()
    await user.click(within(deleteDialog).getByLabelText('我了解风险，确认直接删除目标平台渠道'))
